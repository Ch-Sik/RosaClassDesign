...
public class AttackObject : MonoBehaviour
{
    ...
    LayerMask attackableLayers;                     //attackable한 objects의 layermask
    LayerMask butterflyLayers;                      //butterfly인식을 위한 layermask
    ...
    public void Init(PlayerCombat playerCombat, LayerMask attackableObjects, LayerMask butterfly)
    {
        ...
        this.attackableLayers = attackableObjects;
        this.butterflyLayers = butterfly;
    }
    ...
    {
        //나비와 충돌한다면,
        if ((butterflyLayers.value & 1 << collision.gameObject.layer) > 0)
        {
            Debug.Log("나비와 충돌됨");
            //PlayerCombat에 RideButterFly함수를 실행시키며, 나비의 부모 데이터를 전달한다.
            //나비의 충돌포인트는 나비 자체에 있으나, 나비 프리팹은 웨이포인트 통솔을 위한 부모오브젝트에서 스크립트를 조절하기 때문
            Butterfly butterFlyComponent = collision.GetComponent<Butterfly>();
            if(butterFlyComponent != null && !butterFlyComponent.isCaged) {
